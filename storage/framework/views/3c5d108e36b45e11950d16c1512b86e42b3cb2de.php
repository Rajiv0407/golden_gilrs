<?php $session_data=session()->get('admin_session');
 ...
 $EditEventBooking=checkEditRole($user_per_info,11);
 $statusEventBooking=checkStatusRole($user_per_info,11);

 ?> 
          <div class="carManagement__wrapper">
                <div class="breadcrumbWrapper d-flex align-items-center justify-content-between ">
                    <nav aria-label="breadcrumb">
                        <h3 class="fs-5 m-0 fw-500">Event Booking Request</h3>
                        <ol class="breadcrumb">
                ...
                <div class="filterWrapper">
                    <div class="form filterWrapper__l s_I">
                         ...
                         <div class="form-group">
                            ...
                            <input type="text" class="form-control"  id="s_user_name" placeholder="User Name">
                        </div> 
						<div class="form-group">
                            <label for="Manufacture">Email</label>
                            <input type="text" class="form-control"  id="s_user_email" placeholder="User Email">
                        </div>
						<div class="form-group">
                            <label for="Manufacture">Mobile Number</label>
                            <input type="text" class="form-control"  id="s_user_phone" placeholder="Mobile Number">
                        </div>
						<div class="form-group">
						...
						<div class="form-group">
                            ...
                            <input type="date" class="form-control"  id="s_event_request_date" placeholder="Booking Request date">
                        </div>
                        <div class="form-group">
                            <label for="status">Status</label>  
                            <select name="fStatus_S" id="booking_status_Search" class="form-control">
...
<script type="text/javascript">
     ...
     $(document).ready(function($k){
		 var edit_event_booking_per='<?php echo $EditEventBooking; ?>'
		 var status_event_booking_per='<?php echo $statusEventBooking; ?>'
              ...
              columnDefs: [  
                  {
                      ...
                      "mRender": function (data, type, full) {
						return '<td class="text-center"><a href="'+baseUrl+'/administrator/dashboard#event_booking_detail/'+full['id']+'" onclick="event_booking_detail('+full['id']+')"><i class="bi bi-chevron-right"></i></a></td>';  
					}  
                  } ,
				  {
                    "aTargets": [1],
                ...
                },
                       {
                        ...
                        } ,
                          {
                            ...
                            "mRender" : function(data, type, full){
								 ...
								 if(edit_event_booking_per == 1){
                  if(full['status'] == 1){
									var BookEdit= '<div class="align-items-center d-flex"> <div class="more_n"> <i class="bi bi-three-dots-vertical" type="button" role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="false"></i> <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink"> <li><a class="dropdown-item" href="javascript:void(0);" onclick="ConfirmChangeStatus('+full['id']+')" >Cancel</a></li> <li><a class="dropdown-item" href="javascript:void(0);" onclick="acceptStatus('+full['id']+')" >Accept</a></li></ul> </div> <div> <label class="switch">'
                 }else{

                  var BookEdit='<div class="align-items-center d-flex"><div> <label class="switch">';
                 }
								 }else{
									var BookEdit='<div> <label class="switch">'; 
								 }
                    ...
                    ajax: {
                              url: '<?php echo URL::asset('event_booking_datatable'); ?>',     
                            },  
                     columns : [            
                        { data:'id' },  
                        { data:'event_name'},
    ...
    function cancal_nfor(id){
       ajaxCsrf();
        ...
        if(res.status==1){
          ...
          statusMesage('Event booking cancel successfully','success');  
        }else{
      ...
      ajaxCsrf();
       ...
       if(edit_goodies_title==''){
        ...
        $.ajax({
			...
			if(res.status==1){
			$('#dataTable').DataTable().ajax.reload();
			statusMesage('Event booking Accepted successfully','success');
			}else{
		...
		function clearNForBooking(){  
		   ...
		   function searchForBooking(){
				...
				var bookingStatus=$("#booking_status_Search").val();
				var s_user_phone=$("#s_user_phone").val();
				var s_event_request_date=$("#s_event_request_date").val();
			 ...
			 if(s_user_phone){
				 $('#dataTable').DataTable().column(3).search(s_user_phone).draw();
			 }
			  if(s_event_name){
			 ...
			 if(s_event_request_date){
				 $('#dataTable').DataTable().column(8).search(s_event_request_date).draw();
			 }
...
</script><?php /**PATH C:\xampp\htdocs\golden\resources\views/admin/EventBooking/index.blade.php ENDPATH**/ ?>