...
{ 
    ...
    // select all Users + count how many message are unread from the selected user
       ...
       //  group by users.id ");
        ...
        $chatUser=array_unique(array_merge($unfriendUser,$unfriendUser_,$friendList));
         // echo "<pre>";
         // print_r($otherUser);
         // print_r($unfriendUser);
         // print_r($unfriendUser_);
         // exit ;

        $users = DB::table("users")->select('users.id','users.isOnline',DB::raw("case when users.image is null then concat('".$usrImg."','user_holder.svg') else  concat('".$usrImg."',users.image) end as image"),DB::raw("concat(users.first_name,' ',users.last_name) as name"),'users.email',DB::raw("count(is_read) as unread "))->leftjoin("messages",function($join){
            $join->on('users.id','=','messages.from')->where('is_read',0)->where('messages.to',Auth::id());
        })->where('users.id','!=',Auth::id())
        ->whereIn('users.id',$chatUser)
        ->groupBy('users.id')->get();       
         
        return view('website/chat/index',['users' => $users,'userId'=>$userId,'loginUserId'=>$data['userId']]);
    ...
    $userInfo = User::select('users.id','users.isOnline',DB::raw("case when users.image is null then concat('".$usrImg."','user_holder.svg') else  concat('".$usrImg."',users.image) end as image"),DB::raw("concat(users.first_name,' ',users.last_name) as name"))->where('id',$user_id)->get()->first();
    ...
    $messages = Message::select('messages.id','messages.from','messages.to','messages.message','messages.is_read','messages.created_at','messages.updated_at',DB::raw('users.image as usrImg'))->join('users','users.id','=','messages.from')->where(function($query) use ($user_id, $my_id) {
